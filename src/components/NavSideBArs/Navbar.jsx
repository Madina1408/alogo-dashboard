          ...
          <div className="flex gap-5">
            <div className="flex gap-4 items-center">
              <div className="relative">
                ...
                <div className="bg-skin-fill w-2.5 h-2.5 rounded-full absolute left-4 bottom-3"></div>
              </div>
              <div className="relative">
                <img src={Bell} alt="bell" />
                <div className="bg-skin-fill w-2.5 h-2.5 rounded-full absolute left-3 bottom-3.5"></div>
              </div>
            </div>
            <div  className="flex relative items-center gap-2 font-[700]">
              <img src={Profile} alt="profile picture" />
              <p className="lg:flex hidden">Mohammed Abdullah </p>
              <img onClick={() => setOpen(!open)}  className={`lg:flex hidden cursor-pointer ${open ? 'rotate-180':''}`} src={Arrow} alt="downarrow" />
              { open ? <UserProfile theme={theme}  setTheme={setTheme}/>:''}
            </div>
          </div>
        </div>
      </div>
